...
LICENSE="LGPL-2.1"
SLOT="${PV}"
IUSE="+abi_x86_32 +abi_x86_64 +alsa capi cups custom-cflags dos elibc_glibc fallout4 +fontconfig +gecko gphoto2 gsm gstreamer +jpeg kernel_FreeBSD +lcms ldap +mono mp3 ncurses netapi nls odbc openal opencl +opengl osmesa oss +perl pcap pipelight +png prelink pulseaudio +realtime +run-exes s3tc samba scanner selinux +ssl staging test themes +threads +truetype udev +udisks v4l vaapi wolfenstein2 +X +xcomposite xinerama +xml"
REQUIRED_USE="|| ( abi_x86_32 abi_x86_64 )
	...
	themes? ( staging )
	vaapi? ( staging )
	wolfenstein2? ( staging )" # osmesa-opengl #286560 # X-truetype #551124
...
src_prepare() {
	...
	if use fallout4; then
		#https://bugs.winehq.org/show_bug.cgi?id=41271
		epatch "${FILESDIR}/wine-${PV}-fallout4.patch"
	fi

	if use wolfenstein2; then
		# https://bugs.winehq.org/show_bug.cgi?id=43935
		epatch "${FILESDIR}/wine-${PV}-wolfen2-ntdll.patch"
		epatch "${FILESDIR}/wine-${PV}-wolfen2-vulkan.patch"
	fi
